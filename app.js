...
actualizarSaldo(num);
    ...
    // ==== AJUSTES DE VENTANA CON VISUAL VIEWPORT ====
(function() {
    ...
    function applyViewportHeight() {
        // Usar visualViewport si está disponible (móvil moderno)
        ...
        if (isKeyboardOpen) {
            // Teclado abierto: ajustar posiciones
            const keyboardHeight = window.innerHeight - window.visualViewport.height;
            
            if (buttons) {
                buttons.style.bottom = `${keyboardHeight + 20}px`;
            }
            if (message) {
                message.style.bottom = `${keyboardHeight + 80}px`;
            }
            if (header) {
    ...
    // Eventos para móviles modernos
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', applyViewportHeight);
        window.visualViewport.addEventListener('scroll', applyViewportHeight);
    }
    
    // Eventos fallback para navegadores antiguos
    window.addEventListener('resize', applyViewportHeight);
    window.addEventListener('orientationchange', () => {
        setTimeout(applyViewportHeight, 300);
    });
    
    // Eventos de foco en inputs
