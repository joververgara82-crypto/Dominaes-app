...
// Precargar video de confirmación
...
function precargarVideoConfirmacion() {
  ...
  // Opcional: también cargarlo en un <video> oculto para mayor garantía
...
// Precargar banner fijo de confirmación
let bannerPrecargado = false;
function precargarBannerConfirmacion() {
  if (bannerPrecargado) return;
  
  const bannerUrl = "https://vzgkmunhtwcobukrcovn.supabase.co/storage/v1/object/public/Banner/fixed-confirmacion-01.webp";
  
  // Método 1: <link rel="prefetch">
  const link = document.createElement('link');
  link.rel = 'prefetch';
  link.as = 'image';
  link.href = bannerUrl;
  ...
  // Método 2: cargar en un <img> oculto (más confiable)
...
let numeroPuro = '';
if (numeroDestino === '***') {
  numeroPuro = '***'; // ← Preservar los asteriscos
} else {
...
// ==== ACTUALIZAR EL PLACEHOLDER DEL NOMBRE ====
if (searchInput) {
  const esAsteriscos = numeroPuro === '***';
  
  if (esAsteriscos) {
    // Siempre mostrar "Inserte el nombre" para ***
...
// ═══════════════════════════════════════════════════════════════════════
// ✅ CONTROL DEL VIDEO + OCULTAR AL TERMINAR (SPA)
// ═══════════════════════════════════════════════════════════════════════
...
if (video) {
    ...
    video.play().catch(e =>
        console.warn("Video no pudo reproducirse:", e)
    );
}
}
