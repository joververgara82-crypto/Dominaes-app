...
// ==== MANEJO DEL TECLADO - COPIAR COMPORTAMIENTO DE "INSERTAR NOMBRE" ====
(function() {
    ...
    function moveScreenUp() {
        if (screen) {
            ...
            if (!originalTransform) {
                originalTransform = screen.style.transform || 'none';
            }
            // Mover toda la pantalla hacia arriba (como hace "Inserte el nombre")
    ...
    // Aplicar el mismo comportamiento a los inputs de monto y mensaje
    [amountInput, messageInput].forEach(input => {
        if (input) {
            input.addEventListener('focus', moveScreenUp);
            input.addEventListener('blur', moveScreenDown);
        }
    });
})();
}
