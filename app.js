...
let qrEscaneado = null; // ← Almacena temporalmente el QR

let bannersHomeCargados = false;
let bannersHomeHTML = '';
let currentIndexHome = 0; // ← Índice actual del carrusel
let carruselIntervalo = null; // ← Para limpiar el intervalo

let tempIngresarMontoData = null; // ← AÑADE ESTO AL INICIO DEL ARCHIVO
  ...
  inicio: `
    <div class="entering pantalla-inicio">
      <div class="screen">
        ...
        <div class="content">
          <h1>Envía y recibe dinero</h1>
          <p>Transfiere gratis a toda la comunidad yapera desde tu celular.</p>
  ...
  claveYape: `
    <div class="entering pantalla-clave-yape">
      <div class="content-parent">
        ...
        <div id="pinContainer" style="margin: 130px auto 30px; display: flex; gap: 22px; justify-content: center;">
          <span class="pin"></span>
          <span class="pin"></span>
          <span class="pin"></span>
          <span class="pin"></span>
      ...
      <div class="content">
        <div class="recipient-card">
          <div class="svg-placeholder">
            <img src="assets/mw.png" alt="Yape" />
          </div>
...
<div class="promo-banner" id="promo-banner">
          ...
          <div class="promo-container">
            <img 
  id="promo-img" 
  src="https://vzgkmunhtwcobukrcovn.supabase.co/storage/v1/object/public/Banner/fixed-confirmacion-01.webp" 
  class="banner-img" 
  onerror="this.src='assets/banner18.webp'"
/>
          </div>
        </div>
      </div>
    ...
    <div class="entering pantalla-historial-completo">
      <header class="top-bar">
        ...
        <div class="icons-right">
          ...
          <img src="assets/827.svg">
        </div>
      </header>
      <main class="content" id="historial-content">
...
function initIngresarClave() {
    ...
    setTimeout(() => {
        ...
        }, 2000); // Duración del fondo morado
        
    }, 2000); // Duración del modal "Validando datos"
} else {
...
function initYapearA() {
  ...
  newNumberResult.addEventListener('click', () => {
      // Obtener el valor directamente del input de búsqueda
      const valorBusqueda = searchInput.value.trim();
      // Limpiarlo para obtener solo los dígitos
      const numeroLimpio = valorBusqueda.replace(/\D/g, '');
      
      if (numeroLimpio.length === 9) {
          goToWithTransition('ingresarMonto', { numeroDestino: numeroLimpio });
      } else if (numeroLimpio.length === 3) {
          goToWithTransition('ingresarMonto', { numeroDestino: numeroLimpio });
      }
  });
  // --- FIN DEL EVENTO MODIFICADO ---
  ...
  function renderContacts(list) {
    ...
    list.forEach(contact => {
      ...
      li.addEventListener("click", () => {
        alert(`Seleccionaste: ${contact.name} (${contact.number})`);
      });
...
// Lógica para la pantalla de ingresar monto
function initIngresarMonto(data) {
    // Soportar tanto string (número) como objeto (con nombrePersonalizado)
    let numeroDestino = '';
    ...
    // --- NORMALIZAR EL NÚMERO A 9 DÍGITOS PUROS ---
    const numeroPuro = (numeroDestino || '').toString().replace(/\D/g, '');
    
    // --- RECUPERAR TODOS LOS CONTACTOS PARA ESTE NÚMERO ---
    ...
    (function(){
        ...
        function applyViewportHeight() {
            ...
            if (keyboardOpen) {
                ...
                buttons.style.bottom = (keyboardHeight + 10) + "px";
                message.style.bottom = (keyboardHeight + 70) + "px";
            } else {
                buttons.style.bottom = "4vw";
                message.style.bottom = "22vw";
            }
        }
        ...
        document.querySelectorAll("input, textarea").forEach(el => {
            el.addEventListener("focus", () => setTimeout(applyViewportHeight, 20));
            el.addEventListener("blur", () => setTimeout(applyViewportHeight, 20));
        });
    })();
...
if (yapeLogoImg) {
  // Primero, quitar cualquier clase de tipo anterior
  ...
  if (esHistorial) {
    ...
    yapeLogoImg.classList.add('yape-logo-svg'); // ← Clase específica para SVG
  } else {
    yapeLogoImg.src = 'assets/AnimationYape.gif';
    yapeLogoImg.alt = 'Animación de Yape';
    yapeLogoImg.classList.add('yape-logo-gif'); // ← Clase específica para GIF
  }
}
    ...
    // --- ÚLTIMOS 3 DÍGITOS ---
    let numVisible = '*** *** ***';    if (numero.length >= 3) {
    ...
    if (buttonsContainer && promoBanner) {
        ...
        // Luego, si viene del historial, mostrar los botones
    ...
    if (numeroLimpio.length >= 9) {
        ...
        // Permitir 3 dígitos (se usará como está)
    ...
    // Pasar tanto el número como el nombre del movimiento
    goToWithTransition('ingresarMonto', { 
        numeroDestino: numeroLimpio,
        nombrePersonalizado: nombre // ← Nombre del movimiento original
    });
});
            buttonsContainer.querySelector('.btn-yape.secondary')?.addEventListener('click', () => {
                alert("¿En qué podemos ayudarte?");
            });
// Al final de initConfirmacionDeYapeo
setTimeout(async () => {
    const imageUrl = await cargarBannerDesdeSupabase();
    const promoImg = document.getElementById("promo-img");
    if (imageUrl && promoImg) {
        promoImg.src = imageUrl;
    }
}, 100);
        }
    }
// ═══════════════════════════════════════════════════════════════════════
// ✅ CONTROL DEL VIDEO + OCULTAR AL TERMINAR
// ═══════════════════════════════════════════════════════════════════════
...
if (video) {
    if (esHistorial) {
        ...
        video.loop = false;
        video.playbackRate = 0.89;
        video.currentTime = 0;

        // ✅ OCULTAR EL VIDEO CUANDO TERMINE
        video.onended = () => {
            video.style.display = 'none';
            video.onended = null; // Limpiar listener (opcional)
        };
  ...
  video.oncanplay = () => {
    video.play().catch(e => console.warn("Autoplay bloqueado:", e));
        ...
        // Intentar reproducir
        video.play().catch(e => console.warn("Video no pudo reproducirse:", e));
    }
}
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2. Iniciar la app según el estado del usuario
  try {
    const appStatus = localStorage.getItem('appStatus');
    const savedPin = localStorage.getItem('yapePin');

    if (appStatus === 'ready' && savedPin) {
      goToWithTransition('ingresarClave');
    } else {
