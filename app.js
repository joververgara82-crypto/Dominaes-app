...
// ==== AJUSTES DE VENTANA - VERSIÓN ROBUSTA ====
(function() {
    ...
    function handleKeyboardState(isOpen) {
        if (isKeyboardOpen === isOpen) return; // No hay cambio
        
        isKeyboardOpen = isOpen;
        
        if (isOpen) {
            // Teclado abierto - mover elementos arriba
            if (buttons) {
                buttons.style.position = 'fixed';
                buttons.style.bottom = '20px';
                buttons.style.left = '8.33vw';
                buttons.style.right = '8.33vw';
                buttons.style.zIndex = '1000';
            }
            if (message) {
                message.style.position = 'fixed';
                message.style.bottom = '80px';
                message.style.left = '8.33vw';
                message.style.right = '8.33vw';
                message.style.zIndex = '999';
            }
        } else {
            // Teclado cerrado - posiciones normales
            if (buttons) {
    ...
    function detectKeyboard() {
        ...
        const heightDifference = lastKnownHeight - currentHeight;
        
        // Si la diferencia es significativa, asumimos que el teclado está abierto
        const keyboardThreshold = 100; // pixels
        
        if (heightDifference > keyboardThreshold) {
            handleKeyboardState(true);
        } else if (heightDifference < -50) {
            // La pantalla volvió a su tamaño normal
            handleKeyboardState(false);
            lastKnownHeight = currentHeight;
        }
    }
    
    // Eventos para detectar cambios en la ventana
    let resizeTimer;
    ...
    [amountInput, messageInput].forEach(input => {
        if (input) {
...
function initConfirmacionDeYapeo(datos) {
  ...
  // Al inicio de initConfirmacionDeYapeo
...
if (screen) {
  ...
  if (esHistorial) {
    ...
    screen.classList.add('pago-nuevo');
  }
}
...
// Usa directamente la URL fija
setTimeout(() => {
  const promoImg = document.getElementById("promo-img");
  if (promoImg) {
    promoImg.src = "https://vzgkmunhtwcobukrcovn.supabase.co/storage/v1/object/public/Banner/Screen_20260126_232649.webp";
  }
}, 100);
        }
    }
